...
# Method:  G. E. Scott and K. K. Irikura, Surf. Interf. Anal. 37, 973-977 (2005)
#
...
#
# create input file for geometry optimization (step 1)
#
step = 1
fn_geom = froot + '_geom.gjf'
fgjf = open( fn_geom, 'w' )
print( 'created file ', fn_geom )
header = '%chk='+froot.replace(",", "-")+'.chk\n' + '%nprocs='+nprocs+'\n' + '%mem='+mem+'mw\n'
directive = '# B3LYP/GEN OPT FREQ scf=xqc'
comment = '\nBEB step ' + str(step) + ': geom and freqs for ' + froot + '\n\n'
...
fn_bu = froot + '_bu.gjf'
fgjf = open( fn_bu, 'w' )
print( 'created file ', fn_bu )
directive = '# HF/GEN geom=check IOP(6/81=3) scf=xqc\n'
comment = '\nBEB step ' + str(step) + ': HF B and U for ' + froot + '\n\n'
fgjf.write( header + directive + comment )
basis = specify_basis( elem, step )
fgjf.write( coords[0] + '\n' + basis )
fgjf.close()
...
if ( len(midz) + len(hiz) ) > 0:
    ...
    fn_bupp = froot + '_bupp.gjf'
    fgjf = open( fn_bupp, 'w' )
    print( 'created file ', fn_bupp )
    directive = '# HF/GEN geom=check IOP(6/81=3) pseudo=read scf=xqc\n'
    comment = '\nBEB step ' + str(step) + ': pseudopotential B and U for ' + froot + '\n\n'
    fgjf.write( header + directive + comment )
    basis = specify_basis( elem, step )
    fgjf.write( coords[0] + '\n' + basis )
    fgjf.close()
...
#
step = 4
fn_ept = froot + '_ept.gjf'
fgjf = open( fn_ept, 'w' )
print( 'created file ', fn_ept )
basis, nopp = specify_basis( elem, 4 )
directive = '# EPT(OVGF+P3,ReadOrbitals)/GEN geom=check scf=xqc'
if need_ecp( basis ):
    directive += ' pseudo=read\n'
else:
    directive += '\n'
comment = '\nBEB step ' + str(step) + ': EPT B for ' + froot + '\n\n'
fgjf.write( header + directive + comment )
fgjf.write( coords[0] + '\n' + basis )
...
#
# create multi-part input file for high-level ionization threshold (step 5)
#
step = 5
# sub-step A: neutral molecule
fn_cc = froot + '_cc.gjf'
fgjf = open( fn_cc, 'w' )
...
fgjf.close()
#
# create input files for high-level double ionization threshold (step 6)
